 ...
 * - Фильтры: тип, категория, город, цена
 * - Сортировка: скоро завершатся, новые, цена (возр./убыв.)
 * - Пагинация
...
// Фильтры
$type = isset($_GET['type']) ? (int)$_GET['type'] : 0; // 1 - прямой, 2 - обратный
$category = isset($_GET['category']) ? (int)$_GET['category'] : 0;
$city = isset($_GET['city']) ? trim($_GET['city']) : '';
$minPrice = isset($_GET['min_price']) ? (float)$_GET['min_price'] : 0;
$maxPrice = isset($_GET['max_price']) ? (float)$_GET['max_price'] : 0;
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'ending'; // ending, new, price_asc, price_desc
    ...
    <div class="auction-grid">
        ...
        <?php else: ?>
            <?php foreach ($auctions as $a):
                ...
                $icon = $isDirect ? '🔥' : '🔄';
                ?>
                <a href="/auctions/view.php?id=<?= $a['id'] ?>" class="auction-card">
